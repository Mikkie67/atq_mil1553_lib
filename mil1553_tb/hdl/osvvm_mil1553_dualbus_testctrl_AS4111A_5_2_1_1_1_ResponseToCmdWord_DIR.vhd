...
architecture AS4111A_5_2_1_1_1_ResponseToCmdWord of osvvm_Mil1553_dualbus_testctrl is
...
begin
  ...
  ControlProc: process
  begin
    SetTestName("AS4111A_5_2_1_1_1_ResponseToCmdWord");
    ...
    Log("-----------------", ALWAYS);
    Log("-----------------", ALWAYS);
    Log("AS4111A_5_2_1_1_1_ResponseToCmdWord", ALWAYS);
    Log("-----------------", ALWAYS);
    Log("-----------------", ALWAYS);
    ...
    wait for 0 ns;
    wait for 0 ns;
    TranscriptOpen(OSVVM_RESULTS_DIR & "AS4111A_5_2_1_1_1_ResponseToCmdWord.txt");
  ...
  begin
    ...
    Log(Manager1Id, "Starting Test AS4111A_5_2_1_1_1: Response To CmdWords", ALWAYS);
    Log(Manager1Id, "------------------------------------------------------", ALWAYS);
    ...
    Log(Manager1Id, "AS4111A_5_2_1_1_1_ResponseToCmdWord ", DEBUG);
    Log(Manager1Id, "------------------------------------------------------", DEBUG);
    -- #################################################################################################################
    -- THIS LOOP DESIGN IS FOR FULL COVERAGE OF ALL BC TO RT RX COMMANDS, INCLUDING BROADCASTS
    -- #################################################################################################################
    for RtAddr in 14 to 31 loop -- loop through all RT address
      log(Manager1Id, "#################################################################################################################", ALWAYS);
      log(Manager1Id, "RT " & to_string(RtAddr), ALWAYS);
      log(Manager1Id, "#################################################################################################################", ALWAYS);
      for SubAddr in 1 to 3 loop -- loop through all the subaddresses (except the two mode commands of 00000 and 11111)
        log(Manager1Id, "SA " & to_string(SubAddr), ALWAYS);
        for Length in 0 to 3 loop -- loop for all the length values
          for TxbRx in 0 to 1 loop -- receive and transmit
            wait for 100 ns;
            -- note that a broadcast is not allowed with a RTtransmit command
            if (TxbRx = 1) then
              if (RtAddr = 31) then
                log(Manager1Id, "Broadcast not allowed with RT Transmit command, skipping this combination", DEBUG);
                next;
              end if;
            end if;

            -- Be carefull of the sub addr used in command 0, because the data will be overwritten by the variable command
            ...
            if (RtAddr = MyRtAddr1) or (RtAddr = MyRtAddr2) or (RtAddr = 31) then
              ...
              Step1Cmd := std_logic_vector(to_unsigned(RtAddr, 5)) & std_logic_vector(to_unsigned(TxbRx, 1)) & std_logic_vector(to_unsigned(SubAddr, 5)) & std_logic_vector(to_unsigned(Length, 5));
              wait for 0 ns;
              log(Manager1Id, "TxbRx = " & to_string(std_logic_vector(to_unsigned(TxbRx, 1))), DEBUG);
            else
              -- the addressed RT is not on the network so expect the command from Step1
              Step1Cmd := std_logic_vector(to_unsigned(TempRtAddr, 5)) & '0' & std_logic_vector(to_unsigned(TempSubAddr, 5)) & std_logic_vector(to_unsigned(2, 5));
            end if;
            log(Manager1Id, "Expected Command Word = " & to_hstring(Step1Cmd), DEBUG);
            wait for 0 ns;
           -- v4p formatting off
            --                           RA      TnR     SA           Len.    Bus RT2RT
            MultiComdList.Command(0) <= (TempRtAddr, 0    , TempSubaddr, 2      , 1, '0'); -- Valid receive to RT1
            MultiComdList.Command(1) <= (RtAddr    , TxbRx, SubAddr    , Length , 1, '0'); -- Variable test command
            MultiComdList.Command(2) <= (TempRtAddr, 1    , 0          , 18     , 1, '0'); -- Transmit last command word received by RT1
            MultiComdList.StartAddr <= 0;
            MultiComdList.Length <= 3;
...
-- #################################################################################################################
configuration osvvm_mil1553_dualbus_testctrl_AS4111A_5_2_1_1_1_ResponseToCmdWord of osvvm_mil1553_dualbus_tb is
  for struct
    for TestCntrl_1: osvvm_mil1553_dualbus_testctrl
      use entity mil1553_tb.osvvm_mil1553_dualbus_testctrl(AS4111A_5_2_1_1_1_ResponseToCmdWord);
    end for;
  end for;
end configuration;
