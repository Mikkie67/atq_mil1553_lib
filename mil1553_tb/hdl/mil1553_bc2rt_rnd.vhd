--
-- VHDL Architecture mil1553_tb.osvvm_Mil1553_dualbus_testctrl.mil1553_bc2rt_rnd
-- Test focuses on BC2RT commands
-- Randomized BC2RT (RT receive) command options (using same commands as directed test)
...
architecture mil1553_bc2rt_rnd of osvvm_Mil1553_dualbus_testctrl is
...
begin
  ...
  ControlProc: process
  begin
    SetTestName("mil1553_bc2rt_rnd");
    log("mil1553_bc2rt_rnd", ALWAYS);
    ...
    wait for 0 ns;
    wait for 0 ns;
    TranscriptOpen(OSVVM_RESULTS_DIR & "mil1553_bc2rt_rnd.txt");
  ...
  begin
    ...
    -- #################################################################################################################
    -- THIS LOOP DESIGN IS FOR RANDOM COVERAGE OF BC TO RT RX COMMANDS, INCLUDING BROADCASTS
    ...
    for CmdValuesIndex in 0 to ArrayIndex - 1 loop -- loop through all Command values
      AddBins(Cov, GenBin(CmdValues1(CmdValuesIndex), CmdValues1(CmdValuesIndex)));
    end loop;
    loop
      Src1 := GetRandPoint(Cov);
      CmdValue1_stdv := std_logic_vector(to_unsigned(Src1, 16));
...
-- #################################################################################################################
configuration mil1553_bc2rt_rnd_cfg of osvvm_mil1553_dualbus_tb is
  for struct
    for TestCntrl_1: osvvm_mil1553_dualbus_testctrl
      use entity mil1553_tb.osvvm_mil1553_dualbus_testctrl(mil1553_bc2rt_rnd);
    end for;
  end for;
end configuration;
