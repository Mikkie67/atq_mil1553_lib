...
architecture mil1553_rt2rt_rnd of osvvm_Mil1553_dualbus_testctrl is
...
begin
  ...
  begin
    ...
    SetTestName("mil1553_rt2rt_rnd");
    log("mil1553_rt2rt_rnd", ALWAYS);
    ...
    wait for 0 ns;
    wait for 0 ns;
    TranscriptOpen(OSVVM_RESULTS_DIR & "mil1553_rt2rt_rnd.txt");
  ...
  begin
    ...
    -- #################################################################################################################
    -- THIS DESIGN IS FOR RANDOM COVERAGE OF RT TO RT COMMANDS
    -- 1. RT1 to RT2 
    -- 2. RT2 to RT1 
    -- 3. RT1 to RTX
    -- 4. RT2 to RTX
    ...
    -- #################################################################################################################
    for CmdValuesIndex in 1 to ArrayIndex - 1 loop -- loop through all Command values
      AddBins(Cov, GenBin(CmdValuesIndex, CmdValuesIndex));
    end loop;
    loop
      Src1 := GetRandPoint(Cov);
      CmdValue1_stdv := std_logic_vector(to_unsigned(CmdValues1(Src1), 16));
      CmdValue2_stdv := std_logic_vector(to_unsigned(CmdValues2(Src1), 16));
      ...
      for ActiveBus in 1 to 2 loop -- loop for bus 1 or 2
        CmdWord1 <= std_logic_vector(to_unsigned(CmdValues1(Src1), 16));
        CmdWord2 <= std_logic_vector(to_unsigned(CmdValues2(Src1), 16));
        wait for 0 ns;
...
-- #################################################################################################################
configuration mil1553_rt2rt_rnd_cfg of osvvm_mil1553_dualbus_tb is
  for struct
    for TestCntrl_1: osvvm_mil1553_dualbus_testctrl
      use entity mil1553_tb.osvvm_mil1553_dualbus_testctrl(mil1553_rt2rt_rnd);
    end for;
  end for;
end configuration;
