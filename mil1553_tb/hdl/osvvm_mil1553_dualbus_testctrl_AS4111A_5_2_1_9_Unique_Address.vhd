--
-- VHDL Architecture mil1553_tb.osvvm_mil1553_dualbus_testctrl_AS4111A_5_2_1_9_Unique_Address
-- Unique Address:
-- The purpose of this test is to verify that the UUT can be assigned any unique address from an external connector 
-- on the UUT.
-- #################################################################################################################
architecture AS4111A_5_2_1_9_Unique_Address of osvvm_Mil1553_dualbus_testctrl is
...
begin
  ...
  ControlProc: process
  begin
    SetTestName("AS4111A_5_2_1_9_Unique_Address");
    ...
    Log("-----------------", ALWAYS);
    Log("-----------------", ALWAYS);
    Log("AS4111A_5_2_1_9_Unique_Address", ALWAYS);
    Log("-----------------", ALWAYS);
    Log("-----------------", ALWAYS);
    ...
    wait for 0 ns;
    wait for 0 ns;
    TranscriptOpen(OSVVM_RESULTS_DIR & "AS4111A_5_2_1_9_Unique_Address.txt");
  ...
  begin
    ...
    Log(Manager1Id, "Starting BC side for Test AS4111A_5_2_1_9_Unique Address", ALWAYS);
    Log(Manager1Id, "--------------------------------------------------------------", ALWAYS);
    ...
    -- The purpose of this test is to verify that the UUT can be assigned any unique address from an external connector 
    -- on the UUT.
    -- The following sequence shall be performed for the UUT:
    -- Step 1. Send a valid, legal command to the UUT.
    -- Step 2. Repeat Step 1 thirty-one times with the same command word except use all other
    -- possible bit combinations in the RT address field of the command word.
    -- Step 3. Repeat Step 1 and Step 2 after externally changing the RT address for all
    -- possible combinations from 00000 thru 11110.
    -- Step 4. After externally changing the RT address to simulate a single point address
    -- validation failure (e.g., parity error on the address lines), repeat Step 1 and Step 2.
    -- The pass criteria shall be: 
    -- Step 1 - CS; 
    -- Step 2 - NR for each combinations; 
    -- Step 3 same as Step 1 and Step 2; 
    -- Step 4 - NR for each combination.

    -- NOTE: Power cycling may be required after externally changing the RT address.
    ----------------------------------------------------------------------------------------------------
    Log(Manager1Id, "------------------------------------------------------", ALWAYS);
    Log(Manager1Id, "Test AS4111_5_2_1_8: STEP 1 - Unique Address.         ", ALWAYS);
    Log(Manager1Id, "------------------------------------------------------", ALWAYS);
    wait for 0 ns;
    TestStep <= 1;
    Toggle(MON1);
    ...
    ----------------------------------------------------------------------------------------------------
    -- Set RT addrs to 0 with discretes
    RT1_DiscretesIn.MyRtAddr <= "00000"; -- Set RT1 addr to 0
    RT1_DiscretesIn.MyRtAddrParity <= '1'; -- Set RT1 addr parity to odd
    RT2_DiscretesIn.MyRtAddr <= "00000"; -- Set RT2 addr to 0
    RT2_DiscretesIn.MyRtAddrParity <= '0'; -- Set RT2 addr parity to even --> this will keep RT in reset
    MyRtAddr2 <= to_integer(unsigned(RT2_DiscretesIn.MyRtAddr)); -- Update MyRtAddr2
    wait for 0 ns;
    for j in 0 to 30 loop -- repeat with different RT1 addresses
      RT1_DiscretesIn.MyRtAddr <= std_logic_vector(to_unsigned(j, 5)); -- Set RT1 addr to j
      wait for 0 ns;
      RT1_DiscretesIn.MyRtAddrParity <= not(RT1_DiscretesIn.MyRtAddr(0) xor RT1_DiscretesIn.MyRtAddr(1) xor RT1_DiscretesIn.MyRtAddr(2) xor RT1_DiscretesIn.MyRtAddr(3) xor RT1_DiscretesIn.MyRtAddr(4));
      MyRtAddr1 <= to_integer(unsigned(RT1_DiscretesIn.MyRtAddr)); -- Update MyRtAddr1
      wait for 0 ns;
      Log(Manager1Id, "Rt Addr = " & to_string(RT1_DiscretesIn.MyRtAddr) & " Parity = " & to_string(RT1_DiscretesIn.MyRtAddrParity), ALWAYS);
      wait for 0 ns;
      -- reset the RT to take new address
      wait for 1 us;
      ----------------------------------------------------------------------------------------------------
      TestStep <= 1;
      ...
      for i in 0 to 30 loop
        TestStep <= 2;
        -- skip the address of RT1
        if (i /= MyRtAddr1) then
          --                          RA TnR SA Len Bus RT2RT
          MultiComdList.Command(0) <= (i, 0, 1, 2, 1, '0'); -- BC 2RT receive on bus 1 
          MultiComdList.Length <= 1;
          wait for 0 ns;
          BC_MULTICMD(MultiComdList, MyRtAddr1, MyRtAddr2, MyBcAddr, CmdWord, BC_cpu_bus, BC1_DiscretesOut, SB_RT1_CMD, SB_RT1_TMP, SB_RT2_CMD, SB_RT2_TMP, SB_RT2RT, RT2RT_DestAddr, RT2RT_Busy, BCID);
          wait for 100 ns;
      ...
      wait for (tNRP + tGAP);
      TestStep <= 3;
      -- the changing of RT address will happen at start of the loop.
    end loop;
    ----------------------------------------------------------------------------------------------------
    ----------------------------------------------------------------------------------------------------
    TestStep <= 4;
    RT1_DiscretesIn.MyRtAddr <= "00000"; -- Set RT1 addr to 0
    RT1_DiscretesIn.MyRtAddrParity <= '0'; -- Set RT1 addr parity to even (error condition)
    MyRtAddr1 <= to_integer(unsigned(RT1_DiscretesIn.MyRtAddr)); -- Update MyRtAddr1
    wait for 0 ns;
    --                          RA TnR SA Len Bus RT2RT
    MultiComdList.Command(0) <= (MyRtAddr1, 0, 1, 2, 1, '0'); -- BC 2RT receive on bus 1 
...
-- #################################################################################################################
configuration osvvm_mil1553_dualbus_testctrl_AS4111A_5_2_1_9_Unique_Address of osvvm_mil1553_dualbus_tb is
  for struct
    for TestCntrl_1: osvvm_mil1553_dualbus_testctrl
      use entity mil1553_tb.osvvm_mil1553_dualbus_testctrl(AS4111A_5_2_1_9_Unique_Address);
    end for;
  end for;
end configuration;
