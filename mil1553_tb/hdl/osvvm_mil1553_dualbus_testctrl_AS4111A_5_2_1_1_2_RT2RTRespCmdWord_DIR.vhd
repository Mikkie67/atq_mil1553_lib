...
begin
  ...
  begin
    ...
    -- #################################################################################################################
    for RtAddr1 in 14 to 31 loop
      for RtAddr in 14 to 30 loop -- loop through all RT address (transmitter cannot be broadcast address)
    ...
    ----------------------------------------------------------------------------------------------------
      log(Manager1Id, "################### STARTING DIRECTED TEST ON BUS1 ###################", ALWAYS);
      MultiComdList.ErrBit <= 0;
      MultiComdList.ErrWrd <= 0;
      MultiComdList.ErrInj <= errNone;
                -- v4p formatting off
      ...
      wait for 0 ns;
      wait for 1 ms;
      log(Manager1Id, "################### DONE WITH DIRECTED TEST BUS1 ###################", ALWAYS);
     log(Manager1Id, "################### STARTING DIRECTED TEST ON BUS2 ###################", ALWAYS);
      MultiComdList.ErrBit <= 0;
      MultiComdList.ErrWrd <= 0;
      MultiComdList.ErrInj <= errNone;
                -- v4p formatting off
                --                           RA TnR SA Len Bus RT2RT
            --   MultiComdList.Command(0) <= (15, 0, 1,  1  , 1, '0'); -- Valid receive to RT1
                MultiComdList.Command(0) <= (15, 0, 2,  1  , 2, '1'); -- Variable test command
                MultiComdList.Command(1) <= (16, 1, 2,  1  , 2, '0'); -- Variable test command
                MultiComdList.Command(2) <= (15, 0, 3,  1  , 2, '1'); -- Variable test command
                MultiComdList.Command(3) <= (16, 1, 3,  1  , 2, '0'); -- Variable test command
                MultiComdList.Command(4) <= (31, 0, 4,  1  , 2, '1'); -- Variable test command
                MultiComdList.Command(5) <= (16, 1, 4,  1  , 2, '0'); -- Variable test command
                MultiComdList.Command(6) <= (15, 1, 0, txMC_TransmitLastCmd , 2, '0'); -- Transmit last valid command word received by RT1
                MultiComdList.StartAddr <= 0;
                MultiComdList.Length <= 7;
                -- v4p formatting on
      wait for 0 ns;
      ...
      wait for 0 ns;
      wait for 1 ms;
      log(Manager1Id, "################### DONE WITH DIRECTED TEST BUS2 ###################", ALWAYS);
   -- #################################################################################################################
